 ...
 * envvars).
 *
 * This file is `require_once`'d from many places (bootstrap, middleware,
 * services). require_once only returns the file's `return` value on the
 * *first* inclusion in a request - every later require_once for the same
 * path just returns `true`. So config is exposed via app_config(), a
 * memoized function, rather than relying on the include's return value.
 */
